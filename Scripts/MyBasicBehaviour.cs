...
public class MyBasicBehaviour : MonoBehaviour
{
	public Camera playerCamera;                        // Reference to the third person player camera.
    public Camera firstPersonPlayerCamera;             // Reference to the first person player camera.
    public float turnSmoothing = 10f;                   // Speed of turn when moving to match camera facing.
	public float sprintFOV = 50f;                        // the FOV to use on the camera when player is sprinting.
	public string sprintButton = "Sprint";                // Default sprint button input name.

    public static bool FPView = true;
    ...
    //private CharacterController cc;                                // Reference to the Animator component.
    private MyThirdPersonOrbitCamBasic camScript;         // Reference to the third person camera script.
    private MyThirdPersonOrbitCamBasic camScriptFP;       // Reference to the third person camera script.
	...
	private List<MyGenericBehaviour> behaviours;            // The list containing all the enabled player behaviours.
	private List<MyGenericBehaviour> overridingBehaviours;  // List of current overriding behaviours.
	...
	public MyThirdPersonOrbitCamBasic GetCamScript { get { return camScript; } }
    public MyThirdPersonOrbitCamBasic GetCamScriptFP { get { return camScriptFP; } }
    ...
    //public CharacterController GetCharacterController { get { return cc; } }
	...
	void Awake ()
	{
		...
		behaviours = new List<MyGenericBehaviour> ();
		overridingBehaviours = new List<MyGenericBehaviour>();
		anim = GetComponent<Animator> ();
        //cc = GetComponent<CharacterController>();
		hFloat = Animator.StringToHash("H");
		vFloat = Animator.StringToHash("V");
		camScript = playerCamera.GetComponent<MyThirdPersonOrbitCamBasic> ();
        camScriptFP = firstPersonPlayerCamera.GetComponent<MyThirdPersonOrbitCamBasic>();
	...
	void Update()
	{
        ...
        //cc.SimpleMove(new Vector3(0f, 0f, v));
		...
		if(IsSprinting())
		{
            ...
            camScriptFP.SetFOV(sprintFOV);
        }
		else if(changedFOV)
		{
			camScript.ResetFOV();
            camScriptFP.ResetFOV();
			changedFOV = false;
		}
	...
	void FixedUpdate()
	{
		...
		{
			foreach (MyGenericBehaviour behaviour in behaviours)
			{
		...
		{
			foreach (MyGenericBehaviour behaviour in overridingBehaviours)
			{
				behaviour.LocalFixedUpdate();
			}
		}
    ...
    // Function that switches to 3rd person view
    ...
    // Function that switches to 1st person view
...
public abstract class MyGenericBehaviour : MonoBehaviour
{
	//protected Animator anim;                       // Reference to the Animator component.
	protected int speedFloat;                      // Speed parameter on the Animator.
	protected MyBasicBehaviour behaviourManager;     // Reference to the basic behaviour manager.
	...
	{
		// Set up the references.
		behaviourManager = GetComponent<MyBasicBehaviour> ();
